...
export default class Support {
  ...
  async doRequest(loginPage, actions) {
    if (this.getSessionData() == undefined) {
      let user = Role(loginPage, actions, { preserveUrl: true });
      await this.useRole(user);
    } else {
      let user = this.deSerializeUserRole(this.getSessionData());
      await this.useRole(user);
    ...
    await this.t.addRequestHooks(cookieHook).eval(() => location.reload(true));
